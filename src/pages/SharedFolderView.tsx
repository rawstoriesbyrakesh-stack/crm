...
const IMAGE_CACHE_NAME = 'rawstories-gallery-cache-v1';

async function getCachedImageUrl(src: string): Promise<string> {
  if (!src || !('caches' in window)) return src;
  try {
...
// ── LazyImage: IntersectionObserver-based image component with CacheStorage ──
...
const LazyImage: React.FC<LazyImageProps> = ({ src, alt, className, priority = false, onLoaded, style }) => {
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(
      ...
      { rootMargin: '400px 0px', threshold: 0 } // start loading 400px before entering viewport
    );
  ...
  useEffect(() => {
    ...
    if (shouldLoad) {
      getCachedImageUrl(src).then((cached) => {
        if (isMounted) setDisplaySrc(cached);
      });
    }
...
export default function SharedFolderView() {
  ...
  // ─── Shared helper: get presigned URL then fetch binary blob ─────────────
  // Retries up to 3 times on 429 (rate-limit) with exponential back-off.
  const fetchImageBlob = async (item: any): Promise<{ blob: Blob; fileName: string } | null> => {
    const apiUrl = rawStoriesApiUrl(`/default/downloadimage?key=${encodeURIComponent(item.id)}&shareId=${shareId || ''}`);
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        if (attempt > 0) await new Promise(r => setTimeout(r, 1500 * attempt)); // back-off: 1.5s, 3s
        const apiRes = await fetch(apiUrl);
        if (apiRes.status === 429) { console.warn('Rate limited on', item.id, '- retrying...'); continue; }
        if (!apiRes.ok) { console.error('downloadimage API error', item.id, apiRes.status); return null; }
        const json = await apiRes.json();
        const presignedUrl = json?.url;
        if (!presignedUrl) { console.error('No presigned URL returned for', item.id); return null; }
        // Step 2: fetch binary image from presigned URL
        const imgRes = await fetch(presignedUrl);
        if (!imgRes.ok) { console.error('Image fetch failed', item.id, imgRes.status); return null; }
        const blob = await imgRes.blob();
        const fileName = item.filename || item.id.split('/').pop() || `photo.jpg`;
        return { blob, fileName };
      } catch (err) {
        console.error('fetchImageBlob error attempt', attempt, item.id, err);
      }
    }
    return null;
  };

  // Process itemsToZip in batches of `concurrency` to avoid rate limits
  const buildZip = async (
    ...
    for (let i = 0; i < itemsToZip.length; i += concurrency) {
      ...
      const results = await Promise.all(batch.map(item => fetchImageBlob(item)));
      ...
      if (i + concurrency < itemsToZip.length) {
  ...
  const handleDownloadFavoritesZip = async () => {
    ...
    notify(setNotifications, `📦 Preparing ZIP of ${total} favorite photo(s)...`, 'info');
    try {
      const zip = new JSZip();
      const favItems = items.filter(item => favorites.has(item.id));
      const count = await buildZip(zip, favItems, total, (done) => {
        if (done % 10 === 0 || done === total) {
          notify(setNotifications, `Downloading ${done}/${total}...`, 'info');
        }
      });
      ...
      notify(setNotifications, `✅ Exported ${count} of ${total} favorite photo(s)!`, 'success');
    } catch (err: any) {
  ...
  const handleDownloadSelectedZip = async () => {
    ...
    notify(setNotifications, `📦 Preparing ZIP of ${total} selected photo(s)...`, 'info');
    try {
      const zip = new JSZip();
      const selItems = items.filter(item => selected.has(item.id));
      const count = await buildZip(zip, selItems, total, (done) => {
        if (done % 10 === 0 || done === total) {
          notify(setNotifications, `Downloading ${done}/${total}...`, 'info');
        }
      });
      ...
      saveAs(zipBlob, zipName);
      notify(setNotifications, `✅ Downloaded ${count} of ${total} selected photo(s)!`, 'success');
    } catch (err: any) {
      console.error('Zip download error:', err);
  ...
  // ── Fetch images with 0ms Session Cache ─────────────────────────────────────
  const fetchItems = useCallback(async () => {
    const metaCacheKey = `rawstories_folder_meta_${encodeURIComponent(decoded || '')}_${shareId || ''}`;
    try {
      const cachedStr = sessionStorage.getItem(metaCacheKey);
      if (cachedStr) {
    ...
    <div className="min-h-screen text-slate-100 relative overflow-hidden" style={{ background: '#0b0f19', fontFamily: "'Inter', sans-serif" }}>
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            {favCount > 0 && (
              <button onClick={() => setShowFavoritesOnly(!showFavoritesOnly)}
                className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all border shadow-md ${showFavoritesOnly ? 'bg-rose-500 text-white border-rose-400 shadow-rose-500/30' : 'bg-slate-800/90 border-slate-700 text-rose-300 hover:bg-slate-700'}`}>
            ...
            <button onClick={() => setShowQrModal(true)}
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-400/40 transition-all shadow-md hover:scale-105"
              title="Share QR Code">
                    ...
                    <div className="aspect-square relative bg-slate-950">
                      ...
                      <div className="absolute top-2.5 left-2.5 right-2.5 flex items-start justify-between z-20">
                        <button onClick={e => { e.stopPropagation(); toggleSelect(item.id); }}
                          className={`w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all shadow-md ${isSelected ? 'bg-amber-400 border-amber-400 text-slate-950' : 'bg-slate-950/80 backdrop-blur border-slate-500 text-white hover:border-amber-400'}`}>
                          {isSelected && <Check className="h-4 w-4 stroke-[3]" />}
                        </button>

                        <button onClick={e => { e.stopPropagation(); toggleFavorite(item.id); }}
                          className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all shadow-md ${isFav ? 'bg-rose-500 text-white shadow-rose-500/40' : 'bg-slate-950/80 backdrop-blur border border-slate-500 text-slate-300 hover:text-rose-400 hover:border-rose-400'}`}>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).filter(p => Math.abs(p - currentPage) <= 2 || p === 1 || p === totalPages).map((page, i, arr) => (
                    <React.Fragment key={page}>
                      ...
                      <button onClick={() => paginate(page)}
                        className={`w-10 h-10 rounded-2xl text-sm font-extrabold transition-all border shadow-md ${page === currentPage ? 'text-slate-950 border-amber-400 bg-amber-400 shadow-amber-500/20' : 'text-slate-200 border-slate-700 bg-slate-900 hover:bg-slate-800 hover:border-slate-500'}`}>
                        {page}
                      </button>
                    </React.Fragment>
                  ))}
            ...
            {currentItems.map((item, idx) => {
              ...
              return (
                <div key={item.id} onClick={() => setLightbox(indexOfFirstItem + idx)}
                  className={`group flex items-center gap-4 p-3.5 rounded-2xl border cursor-pointer transition-all shadow-md ${isSel ? 'bg-amber-500/15 border-amber-400' : 'bg-slate-900/90 border-slate-800 hover:bg-slate-800 hover:border-slate-700'}`}>
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-4 shrink-0 bg-slate-900 border-b border-slate-800">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button onClick={() => setShowComments(!showComments)}
                className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold border transition-all shadow-md ${showComments ? 'bg-amber-400 text-slate-950 border-amber-400' : 'bg-slate-800 border-slate-700 text-slate-200 hover:bg-slate-700'}`}>
            ...
            {filteredItems.slice(Math.max(0, lightbox - 4), lightbox + 8).map((item, i) => {
              ...
              return (
                <button key={item.id} onClick={() => setLightbox(gi)}
                  className={`relative w-12 h-12 sm:w-14 sm:h-14 rounded-xl overflow-hidden shrink-0 border-2 transition-all ${gi === lightbox ? 'border-amber-400 scale-110 shadow-lg shadow-amber-400/30' : 'border-slate-800 opacity-60 hover:opacity-100'}`}>
                  {item.isVideo
      ...
      {showQrModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(16px)' }}>
          <div className="relative bg-slate-900 border border-slate-700/80 rounded-3xl p-6 sm:p-8 max-w-sm w-full text-center shadow-2xl overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-500 via-amber-400 to-rose-500" />
            <button onClick={() => setShowQrModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white p-2 rounded-xl bg-slate-800">
              <X className="h-4 w-4" />
            </button>
            <div className="w-12 h-12 bg-amber-500/20 border border-amber-400/40 rounded-2xl flex items-center justify-center mx-auto mb-4 text-amber-400">
              <QrCode className="h-6 w-6" />
            </div>
            <h3 className="text-xl font-bold text-white mb-1">Scan Gallery QR</h3>
            <p className="text-xs text-slate-400 mb-5">Scan to instantly view this photo gallery on your phone</p>

            <div id="gallery-qr-canvas-container" className="bg-white p-4 rounded-2xl inline-block shadow-2xl mb-5">
              <QRCodeCanvas value={window.location.href} size={200} includeMargin bg="#ffffff" fg="#090d16" />
            </div>
            ...
            <div className="space-y-2.5">
              <button onClick={() => {
                ...
                if (canvas) {
                  ...
                  a.click();
                  notify(setNotifications, 'QR Code PNG downloaded!', 'success');
                }
              }} className="w-full py-3 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-extrabold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20">
              ...
              <button onClick={() => {
                ...
                notify(setNotifications, 'Gallery link copied!', 'success');
              }} className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold rounded-xl text-xs flex items-center justify-center gap-2 border border-slate-700">
                {qrCopied ? <Check className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4" />}
                {qrCopied ? 'Copied to Clipboard!' : 'Copy Gallery Link'}
              </button>

              <a href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out this photo gallery: ${window.location.href}`)}`} target="_blank" rel="noopener noreferrer" className="w-full py-2.5 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 font-bold rounded-xl text-xs flex items-center justify-center gap-2 border border-emerald-500/30">
                <MessageCircle className="h-4 w-4 fill-current" /> Share via WhatsApp
              </a>
            </div>
          </div>
        </div>
